...
export function OwnerManagerChat({ ownerId, ownerName, managerId, managerName }: ChatProps) {
  ...
  useEffect(() => {
    ...
    newSocket.on("connect", () => {
      newSocket.emit("joinChat", { role: "owner", userId: ownerId });
    });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogTrigger asChild>
        <Button variant="default" className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white font-medium shadow-sm transition-all flex justify-center items-center gap-2">
      ...
      <DialogContent className="sm:max-w-[450px] h-[600px] flex flex-col p-0 overflow-hidden bg-slate-50">
        <DialogHeader className="p-4 border-b bg-white shadow-sm flex flex-row items-center gap-3">
          ...
          <div>
            <DialogTitle className="text-lg font-bold text-slate-900">{managerName}</DialogTitle>
        ...
        <div className="p-4 bg-white border-t">
          <form onSubmit={sendMessage} className="flex flex-row items-center gap-2">
            ...
            />
            <Button type="submit" size="icon" className="rounded-full bg-blue-600 hover:bg-blue-700 shrink-0 shadow-sm" disabled={!inputText.trim()}>
