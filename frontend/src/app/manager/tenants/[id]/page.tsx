...
export default function ManagerTenantProfilePage() {
  const { id } = useParams();
  ...
  if (!tenant) {
    return (
      <div className="flex h-[80vh] flex-col items-center justify-center space-y-4">
        ...
        <Button onClick={() => router.push('/manager/tenants')} variant="outline">
                  ...
                  {tenant.leases.map((lease) => (
                    <div key={lease.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors hover:bg-slate-50/50">
                      ...
                      <div className="shrink-0 flex gap-2">
                        <Button variant="outline" size="sm" onClick={() => router.push(`/manager/leases/${lease.id}`)}>
