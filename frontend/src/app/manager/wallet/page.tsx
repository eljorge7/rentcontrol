    ...
    const fetchWalletData = async () => {
      try {
        const [sumRes, comRes, payRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/commissions/summary`, { headers: getAuthHeaders() }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/commissions/my-commissions`, { headers: getAuthHeaders() }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/commissions/my-payrolls`, { headers: getAuthHeaders() })
        ]);
