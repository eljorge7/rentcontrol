...
import { AlertCircle, Clock, Wrench, CheckCircle2, ShieldAlert, UserPlus, X, Loader2, DollarSign } from "lucide-react";
...
export default function OwnerIncidentsPage() {
  ...
  const [proposeCostModalOpen, setProposeCostModalOpen] = useState(false);
  const [selectedIncident, setSelectedIncident] = useState("");
  const [selectedSupplier, setSelectedSupplier] = useState("");
  const [supplierCost, setSupplierCost] = useState("");
  const [rcMarkup, setRcMarkup] = useState("0.05");
  ...
  const handleProposeCost = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.patch(`/incidents/${selectedIncident}/propose-cost`, {
        supplierCost: parseFloat(supplierCost),
        rcMarkup: parseFloat(rcMarkup)
      });
      if (res.status === 200) {
        setProposeCostModalOpen(false);
        setSupplierCost("");
        fetchIncidents();
        
        if (res.data.status === 'IN_PROGRESS') {
          alert(`¡Autorizado automáticamente por el sistema! El costo final de $${res.data.finalCharge} MXN es menor al límite de mantenimiento pre-aprobado de esta propiedad.`);
        } else {
          alert(`Cotización enviada al Propietario. El monto supera el límite pre-aprobado, por lo que requiere su autorización explícita.`);
        }
      }
    } catch (error: any) {
      console.error(error);
      alert(error.response?.data?.message || "Error al proponer costo");
    }
  };
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="mt-4 w-full">
                    ...
                    {incident.status === 'IN_PROGRESS' && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => { setSelectedIncident(incident.id); setProposeCostModalOpen(true); }}
                        className="mt-2 w-full text-xs text-amber-700 border-amber-200 hover:bg-amber-50"
                      >
                        <DollarSign className="h-3 w-3 mr-1.5" /> Proponer Cotización
                      </Button>
                    )}
                  </div>
                ) : (
      ...
      {/* Modal Proponer Costo */}
      {proposeCostModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-5 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-900">Proponer Cotización al Dueño</h3>
              <button onClick={() => setProposeCostModalOpen(false)} className="text-slate-400 hover:text-slate-600">
                <X className="h-5 w-5" />
              </button>
            </div>
            
            <form onSubmit={handleProposeCost}>
              <div className="p-5 space-y-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700">Costo del Técnico (Proveedor):</label>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      min="0"
                      step="0.01"
                      className="flex h-10 w-full rounded-md border border-slate-200 bg-white pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-600"
                      value={supplierCost}
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700">Margen RentControl (Markup):</label>
                  <select 
                    required
                    className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-600"
                    value={rcMarkup}
                  ...
                  >
                    <option value="0">0% (Sin margen)</option>
                    <option value="0.05">5% (Estándar)</option>
                    <option value="0.10">10%</option>
                    <option value="0.15">15%</option>
                  </select>
                </div>

                {supplierCost && !isNaN(Number(supplierCost)) && (
                  <div className="p-3 bg-blue-50 border border-blue-100 rounded-lg mt-4 flex justify-between items-center">
                    <span className="text-sm font-medium text-blue-800">Costo Final al Dueño / Inquilino:</span>
                    <span className="text-lg font-bold text-blue-900">
                      ${(Number(supplierCost) * (1 + Number(rcMarkup))).toLocaleString('es-MX', {minimumFractionDigits: 2})} MXN
                    </span>
                  </div>
                )}
                
                <p className="text-xs text-slate-500 mt-2">Al confirmar, el estado pasará a "Esperando Aprobación" y el Propietario decidirá a quién se le cobra.</p>
              </div>
              <div className="p-5 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <Button type="button" variant="outline" onClick={() => setProposeCostModalOpen(false)} className="hover:bg-slate-100 rounded-xl">Cancelar</Button>
                <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl">
                  Enviar Propuesta
                </Button>
              </div>
