...
import { DollarSign, Plus, Edit, Calendar, FileText, ExternalLink, Search } from "lucide-react";
...
export default function PaymentsListPage() {
  ...
  const filteredCharges = charges.filter(c => 
    (c.lease?.tenant?.name || "").toLowerCase().includes(searchTerm.toLowerCase()) || 
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="relative w-full sm:w-64">
            ...
            <Input
              type="text"
              placeholder="Buscar pago o inquilino..."
                ...
                {filteredCharges.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={7} className="h-24 text-center text-slate-500">
                      {charges.length === 0 ? "No hay cargos registrados." : "No se encontró ningún cargo."}
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
